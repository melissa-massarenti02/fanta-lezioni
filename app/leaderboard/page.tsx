          ...
          <div className="flex items-end justify-center gap-4 mb-8">
            ...
            {top3[1] && (
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-xs text-slate-300 font-bold truncate max-w-16 text-center">
                  {top3[1].nome.split(" ")[0]}
                </span>
            ...
            {leader && (
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-xs text-yellow-300 font-bold truncate max-w-20 text-center">
                  {leader.nome.split(" ")[0]}
                </span>
            ...
            {top3[2] && (
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-xs text-amber-600 font-bold truncate max-w-16 text-center">
                  {top3[2].nome.split(" ")[0]}
                </span>
        ...
        <div className="glass rounded-2xl overflow-hidden">
          {entries.length === 0 ? (
            ...
            entries.map((entry, i) => {
              ...
              const distacco = prev
                ? entry.punti_totali - prev.punti_totali
                : 0;
              return (
                <div
                  key={entry.id}
                  className={`flex items-center gap-4 px-4 py-4 border-b border-white/5 last:border-0 transition-colors ${
                ...
                >
                  ...
                  <div className="w-10 h-10 flex-shrink-0">
                    <Avatar src={entry.photoURL} name={entry.nome} size={40} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p
                      className={`font-bold truncate ${isMe ? "text-blue-300" : "text-white"}`}
                    >
                      {entry.nome} {isMe && "(tu)"}
                    </p>
                    {i > 0 && (
                      <p className="text-xs text-red-400">
                        {distacco} pts dal precedente
                      </p>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span
                      className={`font-black text-lg ${isMe ? "text-blue-300" : "text-white"}`}
                    >
                      {entry.punti_totali}
                    </span>
                  </div>
