...
import { Play, Pause, RotateCcw, Brain, Coffee, AlertTriangle } from "lucide-react";
...
export default function TimerPage() {
    ...
    useEffect(() => {
        if (completed && user) {
            (async () => {
                const pts = await updatePoints(user.uid, "studio");
                await refreshUserData();
                setMsg(`🎉 Sessione completata! +${pts} punto studio`);
            })();
        }
    }, [completed, user, refreshUserData]);
    ...
    return (
        <div className="md:pl-20 min-h-screen">
            ...
            <main className="p-4 md:p-8 pb-24 md:pb-8 max-w-lg mx-auto flex flex-col items-center gap-6">
                <h1 className="text-2xl font-black text-white mt-2">Timer Pomodoro 🍅</h1>
                ...
                <div className="flex gap-2 glass rounded-2xl p-1.5 w-full">
                    {MODES.map((m, i) => (
                        <button
                            ...
                            className={`flex-1 py-2 rounded-xl text-sm font-bold transition-all duration-200 ${i === modeIdx ? "bg-white/10 text-white" : "text-slate-400 hover:text-white"
                                }`}
                        >
                            {m.label}
                        </button>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        ...
                        style={{ backgroundColor: accent, boxShadow: `0 0 24px ${accent}60` }}
                    >
                ...
                {running && (
                    <div className="w-full flex items-start gap-3 bg-yellow-400/10 border border-yellow-400/20 rounded-xl p-4">
                        ...
                        <p className="text-yellow-200 text-xs">
                            <strong>Attenzione!</strong> Se esci dall'app o cambi scheda durante il timer, perderai 1 punto per distrazione.
                        </p>
                    </div>
                )}
